...
// File: @eaglevision/matrixswap-lib/contracts/token/BEP20/IBEP20.sol
...
*/
contract EviProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public eviToken;
    ...
    uint256 public numberEviToReactivate;
    uint256 public numberEviToRegister;
    uint256 public numberEviToUpdate;
    ...
    constructor(
        IBEP20 _eviToken,
        uint256 _numberEviToReactivate,
        uint256 _numberEviToRegister,
        uint256 _numberEviToUpdate
    ) public {
        eviToken = _eviToken;
        numberEviToReactivate = _numberEviToReactivate;
        numberEviToRegister = _numberEviToRegister;
        numberEviToUpdate = _numberEviToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends EVI to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer EVI tokens to this contract
        eviToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberEviToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer EVI token to this address
        eviToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberEviToUpdate
    ...
    /**
     * @dev Claim EVI to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        eviToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of EVI to register
     * Callable only by owner admins.
     */
    function updateNumberEvi(
        uint256 newNumberEviToReactivate,
        uint256 _newNumberEviToRegister,
        uint256 _newNumberEviToUpdate
    ) external onlyOwner {
        numberEviToReactivate = newNumberEviToReactivate;
        numberEviToRegister = _newNumberEviToRegister;
        numberEviToUpdate = _newNumberEviToUpdate;
    }
