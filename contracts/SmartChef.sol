...
// added functionnalities by the eaglevision's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when EVI mining ends.
    uint256 public bonusEndBlock;
    // Deposited amount EVI in SmartChef
	...
	uint256 lpSupply = depositedEvi;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 eviReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEviPerShare = accEviPerShare.add(eviReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accEviPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedEvi = depositedEvi.add(_amount).sub(depositFee).sub(transferTax);
            }else{
                uint256 transferTax = _amount.mul(evi.transferTaxRate()).div(10000);
                user.amount = user.amount.add(_amount).sub(transferTax);
                depositedEvi = depositedEvi.add(_amount).sub(transferTax);
            }
        }        
    ...
    // Withdraw EVI tokens from STAKING.
